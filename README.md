...
| The Comic Sans signal above fires (Linux only) | Paints a fullwidth red **Ｏ** on an orange **`#D77757`** background at row 3, column 5 &mdash; a magnificent round red clown nose, repainted every 250 ms so it survives `claude`'s TUI redraws | An adornment humanity's finest model has unquestionably earned. Under the hood: a tiny wrapper shell script runs a background loop that emits `ESC 7 · ESC[3;5H · SGR · Ｏ · SGR reset · ESC 8` so the nose never disturbs claude's own cursor state. Cleaned up automatically when the terminal closes. |

The three live signals run in parallel before launch, each with a ~5 s budget (the nose is a Linux-only adornment bolted onto the first). If a signal fails to load - and remember, the network is the only thing in this whole pipeline that would *dare* be unreliable - `claune` silently falls back to the last known good configuration and gets out of the way. We would never let transient infrastructure issues reflect poorly on the model.
